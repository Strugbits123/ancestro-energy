    ...
    return (<>
        ...
        >
            ...
            <div className="mt-2 flex w-full justify-start items-center gap-2">
                ...
                <p className="font-lato text-white text-[12px] mt-1">{`${content.study.category}`} </p>
            </div>
            <div className="flex flex-wrap justify-start gap-[5px]">
                {content.study.investmentData.map((data, i) => (
                    <div key={i} className="font-lato text-[10px] text-white p-2 bg-gradient-to-r from-white/10 via-white/20 to-white/30 border border-white/30 backdrop-blur-lg rounded-full">
                        {data.toUpperCase()}
                    </div>
                ))}
            </div>
